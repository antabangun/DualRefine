...
def se3_exp(vec):
    ...
    theta = torch.norm(phi, 2, 1, keepdim=True)
    a = phi/theta
    a_skew = torch.zeros((vec.shape[0], 3, 3), device=vec.device)
    a_skew[:, 0, 1] = -a[:, 2, 0]
    a_skew[:, 0, 2] = a[:, 1, 0]
    a_skew[:, 1, 0] = a[:, 2, 0]
    a_skew[:, 1, 2] = -a[:, 0, 0]
    ...
    with torch.cuda.amp.autocast(enabled=False):
        exp_psi_skew = (torch.cos(theta)*torch.eye(3, device=vec.device).unsqueeze(0) +
                        (1-torch.cos(theta))*torch.bmm(a, a.permute(0, 2, 1)) +
                        torch.sin(theta)*a_skew)
        J = ((torch.sin(theta)/theta)*torch.eye(3, device=vec.device).unsqueeze(0) +
             (1-(torch.sin(theta)/theta))*torch.bmm(a, a.permute(0, 2, 1)) +
             (1-torch.cos(theta))/theta*a_skew)
        t = torch.bmm(J, rho.type(J.dtype))

    T = torch.eye(4, device=vec.device).unsqueeze(0).repeat(vec.shape[0], 1, 1)
    T[:, :3, :3] = exp_psi_skew
    T[:, :3, -1:] = t
...
class Project3D(nn.Module):
    ...
    def forward(self, points, K, T):
        ...
        pix_coords[..., 0] = 2*(pix_coords[..., 0]+0.5)/self.width - 1
        pix_coords[..., 1] = 2*(pix_coords[..., 1]+0.5)/self.height - 1
        return pix_coords
