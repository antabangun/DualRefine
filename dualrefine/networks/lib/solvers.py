...
def anderson(f, x0, 
    ...
    bsz, L = x0.shape
    alternative_mode = 'rel' if stop_mode == 'abs' else 'abs'
    X = torch.zeros(bsz, m, L, dtype=torch.float, device=x0.device)
    F = torch.zeros(bsz, m, L, dtype=torch.float, device=x0.device)
    X[:,0], F[:,0] = x0.reshape(bsz, -1), f(x0).reshape(bsz, -1)

    if threshold == 1:
        lowest_xest = F[:, 0].view_as(x0).clone().detach()
        out = {"result": lowest_xest,
            "indexing": [lowest_xest],
            "nstep": 0,
            "abs_trace": [0],
            "rel_trace": [0],
            "threshold": threshold
    ...
    H = torch.zeros(bsz, m+1, m+1, dtype=torch.float, device=x0.device)
    H[:,0,1:] = H[:,1:,0] = 1
    y = torch.zeros(bsz, m+1, 1, dtype=torch.float, device=x0.device)
    y[:,0] = 1
    ...
    for k in range(2, threshold):
        ...
        with torch.cuda.amp.autocast(enabled=False):
            H[:,1:n+1,1:n+1] = torch.bmm(G.float(),G.transpose(1,2).float()) + lam*torch.eye(n, dtype=torch.float,device=x0.device)[None]
            try:
                alpha = torch.solve(y[:,:n+1].float(), H[:,:n+1,:n+1].float())[0][:, 1:n+1, 0]   # (bsz x n)
        ...
        gx = (F[:,k%m] - X[:,k%m])
        abs_diff = torch.abs(gx)
        rel_diff = (abs_diff / (1e-5 + torch.abs(F[:, k % m])))
        abs_diff = abs_diff.mean().item()
        rel_diff = rel_diff.mean().item()
        diff_dict = {'abs': abs_diff,
                     'rel': rel_diff}
        trace_dict['abs'].append(abs_diff)
        trace_dict['rel'].append(rel_diff)
        ...
        if indexing and (k+1) in indexing:
            indexing_list.append(lowest_xest)
            iter_num_list.append(lowest_iter_num)
        ...
        if trace_dict[stop_mode][-1] < eps:
            for _ in range(threshold-1-k):
                trace_dict[stop_mode].append(lowest_dict[stop_mode])
                trace_dict[alternative_mode].append(lowest_dict[alternative_mode])
            if len(indexing_list) == 0:
                indexing_list.append(lowest_xest)
            break

    out = {"result": lowest_xest,
           "indexing": indexing_list,
           "iter_num": iter_num_list,
           "lowest": lowest_dict[stop_mode],
           "nstep": lowest_step_dict[stop_mode],
           "prot_break": False,
           "abs_trace": trace_dict['abs'],
